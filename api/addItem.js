...
async function insertItem(req,res){
    ...
    const insertQuery = "INSERT INTO item(name,brand,category,discription,price,image,date) VALUES(?,?,?,?,?,?,?)"
    try{
        const insertPromise = ()=>{
        ...
        res.status(200).send(`insert successfully with an index id of ${results.insertId} `)
